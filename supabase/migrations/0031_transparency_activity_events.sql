...
begin
  ...
  perform public.record_activity_event(new.buyer_id,'purchase',new.id,new.status,label,coalesce(title,'Resource purchase'),new.approved_by,jsonb_build_object('invoice_number',new.invoice_number,'amount_cents',new.amount_cents));
  if seller is not null then
    perform public.record_activity_event(seller,'purchase',new.id,new.status,label,coalesce(title,'Resource purchase'),new.approved_by,jsonb_build_object('invoice_number',new.invoice_number,'amount_cents',new.amount_cents));
  end if;
  return new;
end; $$;
