...
export async function UserRecentActivity() {
  ...
  return <MinimizableSection id="home-recent-activity" title="Recent activity" description="Your latest notifications and account changes." className="container my-0 rounded-3xl py-0"><section className="rounded-3xl border bg-card p-5 shadow-sm"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-semibold text-primary">Recent activity</p><h2 className="mt-1 text-xl font-bold">What changed recently</h2></div><Button asChild size="sm" variant="outline"><Link href="/notifications">View all</Link></Button></div><div className="mt-4 space-y-2">{notifications.map((n)=><Link key={n.id} href={n.link??"/notifications"} className="block rounded-xl border p-3 hover:border-primary/40"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-semibold">{n.title}</p><p className="mt-1 line-clamp-2 text-xs leading-5 text-muted-foreground">{n.body}</p></div><Clock3 className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground"/></div><p className="mt-2 text-[11px] text-muted-foreground">{new Date(n.created_at).toLocaleString()}</p></Link>)}</div></section></MinimizableSection>;
}
