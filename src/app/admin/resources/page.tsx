...
export default async function AdminResourcesPage({ searchParams }: { searchParams: { q?: string; status?: string } }) {
  ...
  return <div className="space-y-6"><div><p className="text-sm font-semibold text-primary">Catalog</p><h2 className="text-2xl font-bold">Resource management</h2><p className="mt-1 text-sm text-muted-foreground">Search the catalog and inspect any resource, regardless of visibility.</p></div>
    ...
    <div className="space-y-3">{rows.map((file: any) => <Card key={file.id}><CardContent className="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="font-semibold">{file.title}</p><p className="mt-1 text-xs text-muted-foreground">{file.seller?.full_name || "Seller"} · {file.category.replaceAll("_", " ")}</p><div className="mt-2 flex flex-wrap gap-2"><Badge variant="outline" className="capitalize">{file.visibility}</Badge><Badge variant="secondary">{file.pricing_type === "free" ? "Free" : formatBDT(file.price_cents)}</Badge></div></div><div className="grid w-full shrink-0 grid-cols-2 gap-2 sm:w-auto"><Button size="sm" variant="secondary" asChild><a href={`/api/files/${file.id}/admin-view`} target="_blank" rel="noreferrer">View</a></Button><Button size="sm" variant="outline" asChild><a href={`/api/files/${file.id}/admin-download`} target="_blank" rel="noreferrer">Download</a></Button></div></CardContent></Card>)}{!rows.length && <Card><CardContent className="p-10 text-center text-sm text-muted-foreground">No resources found.</CardContent></Card>}</div>
  </div>;
}
