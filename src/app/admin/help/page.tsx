...
import { archiveGuideOverviewItem, archiveGuideSection, archiveHelpItem, restoreGuideOverviewItem, restoreHelpItem, upsertGuideOverviewItem, upsertGuideSection, upsertHelpItem } from "./actions";
...
function GuideOverviewEditForm({ item }: { item?: any }) {
  return <form action={upsertGuideOverviewItem} className="grid gap-3 rounded-2xl border bg-muted/10 p-4">
    <input type="hidden" name="id" value={item?.id ?? ""} />
    <div className="grid gap-3 md:grid-cols-4">
      <label><span className={labelClass}>Slug</span><Input name="slug" defaultValue={item?.slug ?? ""} required placeholder="overview-student-preview" /></label>
      <label><span className={labelClass}>Role scope</span><select name="role_scope" defaultValue={item?.role_scope ?? "general"} className={fieldClass}><option value="general">General</option><option value="student">Student</option><option value="seller">Seller</option><option value="admin">Admin</option></select></label>
      <label><span className={labelClass}>Type</span><select name="kind" defaultValue={item?.kind ?? "capability"} className={fieldClass}><option value="intro">Intro</option><option value="capability">Capability</option><option value="workflow">Workflow</option><option value="access">Access</option><option value="next_step">Next step</option></select></label>
      <label><span className={labelClass}>Required access</span><select name="required_access" defaultValue={item?.required_access ?? "none"} className={fieldClass}><option value="none">Everyone</option><option value="authenticated_student">Logged-in Student</option><option value="verified_student">Verified student (legacy)</option><option value="seller">Seller</option><option value="admin">Admin</option></select></label>
    </div>
    ...
    <label><span className={labelClass}>Benefit</span><textarea name="benefit" defaultValue={item?.benefit ?? ""} className={areaClass} placeholder="এই feature ব্যবহার করলে user কী সুবিধা পাবে?" /></label>
    <div className="grid gap-3 md:grid-cols-2">
    ...
    <div className="grid gap-3 md:grid-cols-3">
      <label><span className={labelClass}>Locked message</span><textarea name="locked_message" defaultValue={item?.locked_message ?? ""} className={areaClass} placeholder="Eligibility না থাকলে কী message দেখাবে?" /></label>
    ...
    <div className="flex flex-wrap justify-end gap-2"><Button type="submit"><Plus className="mr-2 h-4 w-4" />{item ? "Save overview" : "Add overview item"}</Button></div>
  </form>;
}
...
export default async function AdminHelpPage({ searchParams }: { searchParams: { saved?: string } }) {
  ...
  const { data: overview } = await supabase.from("guide_overview_items").select("*").order("sort_order", { ascending: true }).order("created_at", { ascending: false });
  return <div className="space-y-6">
    ...
    <Card><CardHeader><CardTitle className="flex items-center gap-2"><BookOpen className="h-5 w-5 text-primary" />A–Z Guide Overview</CardTitle></CardHeader><CardContent className="space-y-4"><p className="text-sm leading-6 text-muted-foreground">এই section থেকেই User Guide-এর প্রথম অংশের overall summary, Student/Seller/Admin capability map, workflow summary এবং action button content control করতে পারবেন। প্রতিটি item-এর content আলাদা করে publish বা archive করা যায়।</p><GuideOverviewEditForm />{(overview ?? []).map((item) => <details key={item.id} className="rounded-2xl border"><summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="font-semibold">{item.title}</span><Badge variant="outline">{item.role_scope}</Badge><Badge variant="outline">{item.kind}</Badge><Badge variant={item.status === "published" ? "default" : "secondary"}>{item.status}</Badge></div><p className="mt-1 text-xs text-muted-foreground">{item.slug} · access: {item.required_access}</p></div></summary><div className="border-t p-3"><GuideOverviewEditForm item={item} /><div className="mt-3 flex flex-wrap justify-end gap-2">{item.status === "archived" ? <form action={restoreGuideOverviewItem}><input type="hidden" name="id" value={item.id} /><Button type="submit" variant="outline"><RotateCcw className="mr-2 h-4 w-4" />Restore to draft</Button></form> : <form action={archiveGuideOverviewItem}><input type="hidden" name="id" value={item.id} /><Button type="submit" variant="outline"><Archive className="mr-2 h-4 w-4" />Archive</Button></form>}</div></div></details>)}</CardContent></Card>
