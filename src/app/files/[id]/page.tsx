...
function MiniRelatedCard({ file }: { file: ResourceCardData }) {
  return <Link href={`/files/${file.id}`} className="group flex gap-3 rounded-xl border bg-card p-3 transition hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-md"><div className="relative h-20 w-24 shrink-0 overflow-hidden rounded-lg bg-muted">{file.thumbnail_url && /\.(jpe?g|png|webp|gif)(\?|$)/i.test(file.thumbnail_url) ? <Image src={file.thumbnail_url} alt="" fill className="object-cover transition-transform group-hover:scale-105" sizes="96px" /> : <div className="flex h-full items-center justify-center bg-gradient-to-br from-primary/10 to-muted"><FileText className="h-7 w-7 text-muted-foreground" /></div>}</div><div className="min-w-0 flex-1">{file.course_code && <p className="font-mono text-[11px] text-primary">{file.course_code}</p>}<h3 className="mt-0.5 line-clamp-2 text-sm font-semibold group-hover:text-primary">{file.title}</h3><div className="mt-2 flex items-center justify-between gap-2 text-xs text-muted-foreground"><span className="flex items-center gap-1"><Star className="h-3 w-3 fill-amber-400 text-amber-400" />{file.average_rating.toFixed(1)} ({file.reviews_count})</span><span>{file.pricing_type === "free" ? "Free" : formatBDT(file.price_cents)}</span></div></div></Link>;
}

export default async function FileDetailPage({ params, searchParams }: { params: { id: string }; searchParams?: { preview?: string } }) {
  const supabase = createClient();
  const admin = createAdminClient();
  const { data: { user } } = await supabase.auth.getUser();
  const { data: file } = await supabase.from("files").select(`id, seller_id, title, description, table_of_contents, category, file_kind, pricing_type, price_cents, thumbnail_url, preview_storage_path, page_count, file_size_bytes, semester, year, language, created_at, average_rating, reviews_count, downloads_count, views_count, ai_summary, ai_keywords, ai_difficulty, ai_reading_time_minutes, course_id, department_id, visibility, upload_batch_id, courses (course_code, course_name)`).eq("id", params.id).in("visibility", ["published","archived"] as any).single();
  if (!file) notFound();
  const { data: batchRows } = file.upload_batch_id
  ...
  if (user && batchFileIds.length) {
    const { data } = await supabase.from("purchases").select("id, status, rejection_reason, created_at").eq("buyer_id", user.id).in("file_id", batchFileIds).order("created_at", { ascending: false });
    purchaseRows = data ?? [];
  }
  ...
  const [{ data: seller }, { data: purchase }, { data: department }, { data: sameCourseFiles }, { data: relatedFiles }] = await Promise.all([
    ...
    file.course_id ? supabase.from("files").select("id, title, thumbnail_url, pricing_type, price_cents, average_rating, reviews_count, downloads_count, views_count, category, file_kind, course_id, department_id, seller_id, semester, year").eq("course_id", file.course_id).eq("visibility", "published").neq("id", file.id).order("downloads_count", { ascending: false }).limit(12) : Promise.resolve({ data: [] }),
    supabase.from("files").select("id, title, thumbnail_url, pricing_type, price_cents, average_rating, reviews_count, downloads_count, views_count, category, file_kind, course_id, department_id, seller_id, semester, year").eq("visibility", "published").neq("id", file.id).limit(40),
  ]);
  ...
  const canDownloadDirectly = isFree || alreadyPurchased || isOwner;
  const previewUrl = !canDownloadDirectly
    ? file.file_kind === "pdf"
      ? `/api/files/${file.id}/preview`
      : file.file_kind === "image"
        ? `/api/files/${file.id}/preview/image`
        : null
    : `/api/files/${file.id}/view`;
  const previewPageCount = file.page_count && !canDownloadDirectly ? Math.min(file.page_count, Math.max(1, Math.ceil(file.page_count * 0.3))) : null;
  const previewRequested = searchParams?.preview === "1";
  const showInlinePreview = previewRequested && Boolean(previewUrl) && file.file_kind === "pdf" && !alreadyPurchased && !isOwner;
  ...
  const { getBuyerPriceCents } = await import("@/lib/platform-pricing");
  const buyerPriceCents = isFree || isOwner ? file.price_cents : await getBuyerPriceCents(supabase as any, file.id, file.price_cents);
  const platformFeeCents = Math.max(0, buyerPriceCents - file.price_cents);

  const mapCard = (item: any): ResourceCardData => ({ id: item.id, title: item.title, thumbnail_url: item.thumbnail_url, file_kind: item.file_kind, pricing_type: item.pricing_type as FilePricingType, price_cents: item.price_cents, average_rating: Number(item.average_rating ?? 0), reviews_count: item.reviews_count ?? 0, downloads_count: item.downloads_count ?? 0, category: item.category, course_code: course?.course_code ?? null, views_count: item.views_count ?? 0 });
  ...
  return <div className="flex min-h-screen flex-col pb-20 lg:pb-0"><Navbar /><main className="container flex-1 py-6 sm:py-10">
    ...
    <div className="grid gap-7 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-start"><div className="min-w-0">
      <div className="overflow-hidden rounded-2xl border bg-card shadow-sm">
        <div className="relative aspect-[16/9] overflow-hidden bg-muted sm:aspect-[2/1]">
          ...
          {!showInlinePreview && previewUrl && (file.file_kind === "pdf" || file.file_kind === "image") && !isOwner && <div className="absolute inset-0 flex items-center justify-center bg-foreground/10 p-4 backdrop-blur-[1px]"><Link href={(alreadyPurchased || isOwner) ? `/files/${file.id}/viewer` : file.file_kind === "image" ? `/files/${file.id}/viewer?preview=1` : `/files/${file.id}?preview=1`} className="inline-flex items-center gap-2 rounded-xl bg-background/95 px-4 py-2.5 text-sm font-semibold text-foreground shadow-lg transition hover:bg-background"><Eye className="h-4 w-4" /> {alreadyPurchased ? "View resource" : "Preview"}</Link></div>}
          <div className="absolute inset-x-3 top-3 flex flex-wrap items-center justify-between gap-2">{file.visibility === "archived" && <Badge variant="destructive" className="rounded-full">REMOVED FROM MARKETPLACE</Badge>}<Badge className="rounded-full bg-background/90 text-foreground shadow-sm backdrop-blur">{RESOURCE_CATEGORY_LABELS[file.category]}</Badge><div className="flex gap-2"><Badge variant="secondary" className="rounded-full bg-background/90 shadow-sm backdrop-blur">{file.file_kind.toUpperCase()}</Badge>{isFree ? <Badge variant="success" className="rounded-full">FREE</Badge> : <Badge className="rounded-full">{isOwner ? "OWNED" : formatBDT(file.price_cents)}</Badge>}</div></div>
        </div>
        <div className="p-5 sm:p-7">{file.visibility === "archived" && <div className="mb-4 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-sm"><strong>Resource removed from marketplace.</strong> Existing buyers keep their access and purchase history.</div>}<div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">{course?.course_code && <Badge variant="outline" className="font-mono">{course.course_code}</Badge>}{course?.course_name && <span>{course.course_name}</span>}{department?.short_name && <><span>•</span><span>{department.short_name}</span></>}</div><h1 className="mt-3 text-2xl font-bold tracking-tight sm:text-3xl">{file.title}</h1><div className="mt-3 flex flex-wrap items-center gap-x-5 gap-y-2"><StarSummary rating={Number(file.average_rating ?? 0)} count={file.reviews_count ?? 0} /><span className="flex items-center gap-1.5 text-sm text-muted-foreground"><Download className="h-4 w-4" />{file.downloads_count} downloads</span><span className="flex items-center gap-1.5 text-sm text-muted-foreground"><Eye className="h-4 w-4" />{file.views_count} views</span></div><div className="mt-5 flex flex-wrap gap-2">{file.semester && <Badge variant="secondary">{file.semester}{file.year ? ` ${file.year}` : ""}</Badge>}{file.language && <Badge variant="secondary">{file.language.toUpperCase()}</Badge>}{file.ai_difficulty && <Badge variant="secondary">{file.ai_difficulty} difficulty</Badge>}{file.ai_reading_time_minutes && <Badge variant="secondary">~{file.ai_reading_time_minutes} min read</Badge>}</div></div>
      </div>

      <div className="mt-5 rounded-2xl border bg-card p-4 shadow-sm sm:hidden">
        <div className="mb-2 flex items-center justify-between gap-3">
          <div><p className="text-xs font-semibold uppercase tracking-wide text-primary">Quick action</p><p className="text-sm font-semibold">Get access before reading more</p></div>
          <Badge variant="outline">{file.file_kind.toUpperCase()}</Badge>
        </div>
        <ResourceDetailActions fileId={file.id} isFree={isFree} alreadyPurchased={alreadyPurchased} isOwner={isOwner} paymentPending={paymentPending} paymentRejected={paymentRejected} hasPreview={Boolean(previewUrl)} compact />
      </div>

      <details open className="group mt-5 rounded-2xl border bg-card p-5 sm:mt-7 sm:p-7"><summary className="flex cursor-pointer list-none items-center justify-between gap-4"><h2 className="text-xl font-bold">About this resource</h2><span className="rounded-full border px-2.5 py-1 text-[11px] font-semibold text-muted-foreground group-open:text-primary">Tap to collapse</span></summary><div className="mt-3"><p className="whitespace-pre-line text-sm leading-7 text-muted-foreground">{file.description || "The seller has not added a description yet."}</p>{file.ai_summary && <div className="mt-5 rounded-xl border border-primary/20 bg-primary/5 p-4"><p className="text-sm font-semibold text-primary">Quick summary</p><p className="mt-1 text-sm leading-6 text-muted-foreground">{file.ai_summary}</p></div>}{!!file.ai_keywords?.length && <div className="mt-5 flex flex-wrap gap-2">{file.ai_keywords.slice(0, 10).map((keyword) => <Badge key={keyword} variant="outline">#{keyword}</Badge>)}</div>}</div></details>
      ...
      {batchFileIds.length > 1 && <section className="mt-5 rounded-2xl border bg-card p-5 sm:p-7"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wide text-primary">One resource · multiple files</p><h2 className="mt-1 text-xl font-bold">This resource includes {batchFileIds.length} files</h2><p className="mt-1 text-sm text-muted-foreground">Buy once and access every attached file in this upload.</p></div><Badge variant="outline">{batchFileIds.length} files</Badge></div><div className="mt-4 space-y-2">{(batchRows ?? []).map((row) => { const fileName = row.storage_path ? row.storage_path.split("/").pop() : `${row.file_kind?.toUpperCase() || "FILE"} file`; return <div key={row.id} className="flex flex-col gap-2 rounded-xl border bg-muted/10 p-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 items-center gap-3"><FileText className="h-4 w-4 shrink-0 text-primary"/><div className="min-w-0"><p className="truncate text-sm font-semibold">{fileName}</p><p className="text-xs text-muted-foreground">{row.file_kind?.toUpperCase() || "FILE"}{row.file_size_bytes ? ` · ${formatBytes(row.file_size_bytes)}` : ""}</p></div></div><div className="flex shrink-0 gap-2"><Button asChild size="sm" variant="outline"><Link href={`/files/${row.id}`}>Preview</Link></Button>{(isFree || alreadyPurchased || isOwner) && <Button asChild size="sm"><a href={`/api/files/${row.id}/download`}>Download</a></Button>}</div></div>})}</div></section>}
    ...
    <aside className="lg:sticky lg:top-24"><div className="rounded-2xl border bg-card p-5 shadow-sm sm:p-6"><div className="mb-1 flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wide text-primary">Resource access</p><p className="mt-1 text-xs text-muted-foreground">Preview before purchase when available.</p></div><InfoButton slug="resource_access" title="Resource Access"><div className="space-y-3"><p>Published resources can show a safe preview before purchase.</p><ul className="space-y-2 pl-5" style={{ listStyleType: "disc" }}><li>Free resources can be viewed and downloaded without purchase.</li><li>Paid resources expose only a limited preview until payment is approved.</li><li>After approval, the original file becomes available through protected access.</li><li>Uploaded resource owners can access their own resources without self-purchasing.</li></ul></div></InfoButton></div><div className="flex items-start justify-between gap-4"><div><p className="text-3xl font-bold">{isFree ? "Free" : isOwner ? "Your resource" : formatBDT(buyerPriceCents)}</p></div><StarSummary rating={Number(file.average_rating ?? 0)} count={file.reviews_count ?? 0} /></div>{!isFree && !isOwner && !alreadyPurchased && <div className="mt-4 rounded-xl border bg-muted/20 p-4"><div className="space-y-1 text-sm"><div className="flex justify-between"><span className="text-muted-foreground">Seller price</span><span className="font-medium">{formatBDT(file.price_cents)}</span></div><div className="flex justify-between"><span className="text-muted-foreground">Platform fee</span><span className="font-medium">{formatBDT(platformFeeCents)}</span></div><div className="flex justify-between border-t pt-2"><span className="font-semibold">Buyer pays</span><span className="font-bold">{formatBDT(buyerPriceCents)}</span></div></div></div>}{(alreadyPurchased || isOwner) && <div className="mt-4 rounded-xl border border-primary/30 bg-primary/5 p-4"><div className="flex items-start gap-3"><CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-primary" /><div><p className="font-semibold">You own this resource</p><p className="mt-1 text-sm text-muted-foreground">{isOwner ? "You uploaded this resource. View or download it anytime." : "Payment approved. View or download it anytime."}</p></div></div></div>}{paymentPending && <div className="mt-4 rounded-xl border border-amber-500/40 bg-amber-500/10 p-4"><div className="flex items-start gap-3"><Clock3 className="mt-0.5 h-5 w-5 shrink-0 text-amber-500" /><div><p className="font-semibold">Payment pending</p><p className="mt-1 text-sm text-muted-foreground">Waiting for admin verification. Please do not pay again.</p></div></div></div>}{paymentRejected && <div className="mt-4 rounded-xl border border-destructive/30 bg-destructive/5 p-4"><div className="flex items-start gap-3"><XCircle className="mt-0.5 h-5 w-5 shrink-0 text-destructive" /><div><p className="font-semibold">Payment needs attention</p><p className="mt-1 text-sm text-muted-foreground">{rejectionReason || "The previous payment could not be verified."}</p></div></div></div>}{!canDownloadDirectly && previewUrl && <div className="mt-4 rounded-xl border bg-muted/20 p-4"><div className="flex items-start gap-3"><Lock className="mt-0.5 h-5 w-5 text-muted-foreground" /><div><p className="text-sm font-semibold">Free preview available</p><p className="mt-1 text-xs text-muted-foreground">{previewPageCount && file.page_count ? `${previewPageCount} of ${file.page_count} pages` : "Sample preview"} before purchase.</p><Link href={`/files/${file.id}/viewer?preview=1`} className="mt-2 inline-flex text-xs font-semibold text-primary underline">Open preview viewer</Link></div></div></div>}<ResourceDetailActions fileId={file.id} isFree={isFree} alreadyPurchased={alreadyPurchased} isOwner={isOwner} paymentPending={paymentPending} paymentRejected={paymentRejected} hasPreview={Boolean(previewUrl)} /><div className="mt-5 border-t pt-4"><ReportResourceButton fileId={file.id} /></div></div>

    </aside>
    </div>
  </main><StickyResourceActionBar fileId={file.id} isFree={isFree} alreadyPurchased={alreadyPurchased} isOwner={isOwner} paymentPending={paymentPending} paymentRejected={paymentRejected} price={buyerPriceCents} hasPreview={Boolean(previewUrl)} /><Footer /></div>;
}
