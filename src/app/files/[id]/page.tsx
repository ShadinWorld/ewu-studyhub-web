    ...
    <div className="grid gap-7 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-start"><div className="min-w-0">
      <div className="overflow-hidden rounded-2xl border bg-card shadow-sm">
        <div className="relative aspect-[16/9] overflow-hidden bg-muted sm:aspect-[2/1]">
          ...
          <div className="absolute inset-x-3 top-3 flex flex-wrap items-center justify-between gap-2"><Badge className="rounded-full bg-background/90 text-foreground shadow-sm backdrop-blur">{RESOURCE_CATEGORY_LABELS[file.category]}</Badge><div className="flex gap-2"><Badge variant="secondary" className="rounded-full bg-background/90 shadow-sm backdrop-blur">{file.file_kind.toUpperCase()}</Badge>{isFree ? <Badge variant="success" className="rounded-full">FREE</Badge> : <Badge className="rounded-full">{isOwner ? "OWNED" : formatBDT(file.price_cents)}</Badge>}</div></div>
        </div>
