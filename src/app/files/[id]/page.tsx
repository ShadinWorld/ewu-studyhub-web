...
export default async function FileDetailPage({ params, searchParams }: { params: { id: string }; searchParams?: { preview?: string } }) {
  ...
  const { data: file } = await supabase.from("files").select(`id, seller_id, title, description, table_of_contents, category, file_kind, pricing_type, price_cents, thumbnail_url, preview_storage_path, page_count, file_size_bytes, semester, year, language, created_at, average_rating, reviews_count, downloads_count, views_count, ai_summary, ai_keywords, ai_difficulty, ai_reading_time_minutes, course_id, department_id, visibility, courses (course_code, course_name)`).eq("id", params.id).in("visibility", ["published","archived"] as any).single();
  if (!file) notFound();
  ...
  const { data: archivedPurchase } = user && file.visibility === "archived" ? await supabase.from("purchases").select("id,status").eq("file_id", file.id).eq("buyer_id", user.id).eq("status", "completed").maybeSingle() : { data: null };
