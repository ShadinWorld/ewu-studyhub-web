...
export default function ToolsPage() {
  return <div className="flex min-h-screen flex-col"><Navbar /><main className="container flex-1 py-8 sm:py-12">
    ...
    <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{tools.map(({href,title,text,icon:Icon})=><Link key={href} href={href} className="group"><Card className="h-full transition-all hover:-translate-y-1 hover:border-primary/40 hover:shadow-lg"><CardContent className="p-5"><div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-primary/10 text-primary"><Icon className="h-5 w-5" /></div><h2 className="mt-4 font-semibold group-hover:text-primary">{title}</h2><p className="mt-1 text-sm leading-6 text-muted-foreground">{text}</p></CardContent></Card></Link>)}</div>
    <div className="mt-8 rounded-2xl border bg-primary/5 p-5"><div className="flex items-start gap-3"><GraduationCap className="mt-0.5 h-5 w-5 shrink-0 text-primary"/><div><p className="font-semibold">Built around EWU's semester flow</p><p className="mt-1 text-sm text-muted-foreground">Admin can replace academic-calendar and final-exam PDFs whenever EWU publishes a new term.</p></div></div></div>
  </main><Footer /></div>;
}
