...
export default async function UploadPage() {
  ...
  const { data: profile } = await supabase.from("profiles").select("is_seller, role, seller_bkash_number").eq("id", user.id).single();
  ...
  const { data: paymentSettings } = await supabase
    ...
    .maybeSingle();
  const hasBkash = isAdminUploader || Boolean(paymentSettings?.bkash_number || profile?.seller_bkash_number);
  ...
  return (
    <div className="container max-w-2xl py-10">
      ...
      {!isAdminUploader && !hasBkash && (
        <div className="mb-8 flex flex-col gap-3 rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-semibold text-amber-800 dark:text-amber-200">Free Resource এখনই upload করতে পারবেন</p>
              <p className="mt-1 text-sm leading-6 text-muted-foreground">
                Paid Resource বিক্রি করে earnings পেতে আগে একটি bKash payout number যোগ করতে হবে। Free Resource upload করতে bKash number এখনই প্রয়োজন নেই।
              </p>
            </div>
          </div>
      ...
      <UploadForm departments={departments ?? []} courses={courses ?? []} hasBkash={hasBkash} />
    </div>
  );
}
