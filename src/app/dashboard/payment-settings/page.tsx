...
import { saveBkashNumber, requestPayout } from "./actions";
...
export default async function PaymentSettingsPage() {
  ...
  return <div className="container max-w-3xl py-10 space-y-6">
    ...
    <Card><CardHeader><CardTitle>Request payout</CardTitle><CardDescription>Available balance: {formatBDT(profile.wallet_balance_cents)} • Minimum payout: BDT 20</CardDescription></CardHeader><CardContent><form action={requestPayout} className="flex gap-3"><div className="flex-1 space-y-2"><Label htmlFor="amount_bdt">Amount (BDT)</Label><Input id="amount_bdt" name="amount_bdt" type="number" min="20" step="1" max={profile.wallet_balance_cents / 100} required /></div><Button type="submit" className="mt-8">Request payout</Button></form></CardContent></Card>
    <Card><CardHeader><CardTitle>Payout history</CardTitle></CardHeader><CardContent>{payouts?.length ? <div className="divide-y">{payouts.map(p => <div key={p.id} className="flex items-center justify-between py-3"><div><p className="font-medium">{formatBDT(p.amount_cents)}</p><p className="text-xs text-muted-foreground">bKash • {p.payment_account_number}</p></div><span className="text-sm capitalize">{p.status}</span></div>)}</div> : <p className="text-sm text-muted-foreground">No payout requests yet.</p>}</CardContent></Card>
  </div>;
}
