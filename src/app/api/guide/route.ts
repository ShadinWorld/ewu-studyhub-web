...
export async function GET() {
  ...
  const [{ data: profile }, { data: sections, error }, { data: overview, error: overviewError }] = await Promise.all([
    ...
    supabase.from("guide_overview_items").select("id, slug, role_scope, kind, title, summary, benefit, action_label, action_href, required_access, locked_message, locked_action_label, locked_action_href, sort_order, updated_at").eq("status", "published").order("sort_order", { ascending: true }),
  ]);

  if (error || overviewError) return NextResponse.json({ error: "Unable to load the User Guide." }, { status: 500 });
  ...
  const verifiedStudent = profile?.student_id_verification_status === "verified" || profile?.role === "verified_student" || isSeller || role === "admin";
  ...
  const visibleOverview = (overview ?? []).filter((item) => {
    ...
    if (item.role_scope === "seller") return true;
    if (item.role_scope === "student") return true;
    return true;
  });
