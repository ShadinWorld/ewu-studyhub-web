...
export default async function SearchPage({ searchParams }: { searchParams: Record<string, string | undefined> }) {
  ...
  let query = supabase.from("files").select("id, title, thumbnail_url, pricing_type, price_cents, average_rating, reviews_count, downloads_count, views_count, category, course_id, seller_id, published_at", { count: "exact" }).eq("visibility", "published").order("published_at", { ascending: false });
  if (q) {
  ...
  return <div className="flex min-h-screen flex-col"><Navbar /><main className="container flex-1 py-8 pb-24 md:py-10 md:pb-10"><div className="max-w-3xl"><div className="flex items-center gap-2 text-sm font-semibold text-primary"><SearchIcon className="h-4 w-4" />Course & resource search</div><h1 className="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">Find what you need faster.</h1><p className="mt-2 text-muted-foreground">Search one box using a course code, course name, resource title, department, or keyword.</p></div><form action="/search" className="mt-7 flex max-w-3xl gap-2 rounded-2xl border bg-card p-2 shadow-sm"><div className="relative min-w-0 flex-1"><SearchIcon className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input name="q" defaultValue={q} placeholder="Try ACT201, Database Systems, notes…" className="h-11 border-0 pl-9 shadow-none focus-visible:ring-0" /></div><Button type="submit" size="lg" className="h-11"><SearchIcon className="h-4 w-4" />Search</Button></form><div className="mt-8 flex items-end justify-between gap-3"><div><p className="text-sm font-medium">{count ?? 0} resources</p><p className="text-xs text-muted-foreground">{q ? `Results for “${q}”` : "Latest published resources"}</p></div>{q && <Button variant="ghost" size="sm" asChild><Link href="/search">Clear</Link></Button>}</div><div className="mt-5"><ResourceCardGrid files={resources} /></div>{totalPages > 1 && <div className="mt-8 flex items-center justify-center gap-3"><Button variant="outline" size="sm" disabled={params.page <= 1} asChild={params.page > 1}>{params.page > 1 ? <Link href={pageHref(params.page - 1)}>Previous</Link> : <span>Previous</span>}</Button><span className="text-sm text-muted-foreground">Page {params.page} of {totalPages}</span><Button variant="outline" size="sm" disabled={params.page >= totalPages} asChild={params.page < totalPages}>{params.page < totalPages ? <Link href={pageHref(params.page + 1)}>Next</Link> : <span>Next</span>}</Button></div>}</main><Footer /></div>;
}
