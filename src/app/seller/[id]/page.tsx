...
export default async function SellerProfilePage({ params }: { params: { id: string } }) {
  ...
  const { data: files } = await admin.from("files").select("id, title, thumbnail_url, pricing_type, price_cents, average_rating, reviews_count, downloads_count, views_count, category, course_id, seller_id").eq("seller_id", seller.id).eq("visibility", "published").order("downloads_count", { ascending: false }).limit(40);
